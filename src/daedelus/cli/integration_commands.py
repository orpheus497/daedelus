...
def shell_integration(shell: str) -> None:
    ...
    try:
        ...
        click.echo("Error: Integration file not found", err=True)
...
def info(ctx: click.Context) -> None:
    ...
    from daedelus import __creator__, __formal_name__, __purpose__, __social_name__, __version__
    ...
    click.echo(f"\n📦 Version: {__version__}")
    click.echo("📄 License: MIT (100% FOSS)")
    click.echo("\n🗂️  Configuration:")
    ...
    if llm_model_path.exists():
        ...
        click.echo("  Hint: Download a GGUF model with: daedelus model download")
...
def doctor(ctx: click.Context) -> None:
    ...
    for shell in ["bash", "zsh", "fish"]:
        try:
            result = subprocess.run(
                ["daedelus", "shell-integration", shell], capture_output=True, text=True, timeout=5
            )
    ...
    if config.data_dir.exists():
        ...
        if os.access(config.data_dir, os.W_OK):
            click.echo("  ✅ Writable: Yes")
        else:
            click.echo("  ❌ Writable: No")
    ...
    if is_daemon_running(config):
        ...
        click.echo("  ⚠️  Daemon: Not running")
    ...
    if db_path.exists():
        ...
        click.echo("  ⚠️  Database not found (will be created on first use)")
    ...
    if socket_path.exists():
        ...
        click.echo("  ⚠️  Socket not found (daemon not running)")
    ...
    if not integrated:
        click.echo("  ⚠️  No shell integration found in RC files")
