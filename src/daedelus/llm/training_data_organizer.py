...
class TrainingDataset:
    ...
    def filter_by_quality(
        self, min_quality: TrainingDataQuality = TrainingDataQuality.MEDIUM
    ) -> list[TrainingExample]:
        """Filter examples by quality threshold"""
        quality_order = [
            TrainingDataQuality.HIGH,
            TrainingDataQuality.MEDIUM,
            TrainingDataQuality.LOW,
            TrainingDataQuality.EXCLUDED,
        ]
            ...
            for row in conn.execute(query):
                ...
                examples.append(
                    TrainingExample(
                        ...
                        context=f"Working directory: {row['cwd']}",
                        metadata={"exit_code": row["exit_code"], "duration": row["duration"]},
...
class FileOperationsFormatter:
    ...
    def format_file_operation_examples(self, limit: int | None = None) -> list[TrainingExample]:
...
class ToolExecutionFormatter:
    ...
    def format_tool_execution_examples(self, limit: int | None = None) -> list[TrainingExample]:
            ...
            for row in conn.execute(query):
                examples.append(
                    TrainingExample(
                        ...
                        metadata={"tool_name": row["tool_name"], "duration": row["duration"]},
                        quality=TrainingDataQuality.MEDIUM,
                        timestamp=row["timestamp"],
                        session_id=row["session_id"],
...
class TrainingDataOrganizer:
    ...
    def collect_all_training_data(
        ...
        limit_per_source: int | None = 1000,
    ) -> TrainingDataset:
        """
        ...
        if include_commands and self.history_db.exists():
            ...
            all_examples.extend(
                seq_examples[:limit_per_source] if limit_per_source else seq_examples
            )
        ...
        if include_file_ops and self.file_ops_db.exists():
            ...
            file_examples = self.file_formatter.format_file_operation_examples(
        ...
        if include_tools and self.tool_db.exists():
            ...
            tool_examples = self.tool_formatter.format_tool_execution_examples(
                limit=limit_per_source
            )
    ...
    def _collect_document_examples(self, limit: int | None = None) -> list[TrainingExample]:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        if self.tool_db.exists():
            with sqlite3.connect(self.tool_db) as conn:
                ...
                successful_execs = conn.execute(
                    "SELECT COUNT(*) FROM tool_executions WHERE success = 1"
                ).fetchone()[0]

                stats["sources"]["tools"] = {"total": total_execs, "successful": successful_execs}
